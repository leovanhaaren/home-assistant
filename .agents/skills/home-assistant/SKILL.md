---
name: home-assistant
description: Manage Home Assistant configuration, deployment workflows, remote CLI access, automation verification, and Lovelace dashboard management.
---
...
- Remote Home Assistant instance management via SSH and pass-cli run -- hass-cli
...
1. SSH access to Home Assistant instance (`root@192.168.1.134`)
2. `pass-cli run -- hass-cli` installed locally
...
### Using pass-cli run -- hass-cli (Local, via REST API)

All `pass-cli run -- hass-cli` commands use environment variables automatically:

```bash
# List entities
pass-cli run -- hass-cli state list

# Get specific state
pass-cli run -- hass-cli state get sensor.entity_name

# Call services
pass-cli run -- hass-cli service call automation.reload
pass-cli run -- hass-cli service call automation.trigger --arguments entity_id=automation.name
```
...
# Tail logs with grep
ssh root@192.168.1.134 "ha core logs | grep -i error | tail -20"
```
...
# 5. Reload or restart
pass-cli run -- hass-cli service call automation.reload  # if reload sufficient
# OR
ssh root@192.168.1.134 "ha core restart"  # if restart needed

# 6. Verify
pass-cli run -- hass-cli state get sensor.new_entity
ssh root@192.168.1.134 "ha core logs | grep -i error | tail -20"
```
...
# 1. Make changes locally
# 2. Quick deploy
scp automations.yaml root@192.168.1.134:/config/
...
- ✅ Automations: `pass-cli run -- hass-cli service call automation.reload`
- ✅ Scripts: `pass-cli run -- hass-cli service call script.reload`
- ✅ Scenes: `pass-cli run -- hass-cli service call scene.reload`
- ✅ Template entities: `pass-cli run -- hass-cli service call template.reload`
- ✅ Groups: `pass-cli run -- hass-cli service call group.reload`
- ✅ Themes: `pass-cli run -- hass-cli service call frontend.reload_themes`
...
```bash
sleep 3
ssh root@192.168.1.134 "ha core logs | grep -i 'automation_name' | tail -20"
```
...
**For device control:**
```bash
pass-cli run -- hass-cli state get switch.device_name
```

**For sensors:**
```bash
pass-cli run -- hass-cli state get sensor.new_sensor
```
...
scp .storage/lovelace.control_center root@192.168.1.134:/config/.storage/
...
scp .storage/lovelace.new_dashboard root@192.168.1.134:/config/.storage/
scp .storage/lovelace_dashboards root@192.168.1.134:/config/.storage/
...
**4. Verify Entities:**
```bash
pass-cli run -- hass-cli state get binary_sensor.front_door
```
...
# Logs
ssh root@192.168.1.134 "ha core logs | tail -50"
ssh root@192.168.1.134 "ha core logs | grep -i error | tail -20"

# State/Services
pass-cli run -- hass-cli state list
pass-cli run -- hass-cli state get entity.name
pass-cli run -- hass-cli service call automation.reload
pass-cli run -- hass-cli service call automation.trigger --arguments entity_id=automation.name
...
ssh root@192.168.1.134 "cd /config && git pull"
scp file.yaml root@192.168.1.134:/config/

# Dashboard deployment
scp .storage/lovelace.my_dashboard root@192.168.1.134:/config/.storage/
...
pass-cli run -- hass-cli service call automation.reload
pass-cli run -- hass-cli service call automation.trigger --arguments entity_id=automation.name
ssh root@192.168.1.134 "ha core logs | grep -i 'automation' | tail -10"
```
